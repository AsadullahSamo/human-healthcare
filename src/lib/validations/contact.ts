...
const phoneSchema = z
  ...
  .optional()
  .refine(
    v => !v?.trim() || /^(\+\d{1,3}[\s\-]?)?\(?\d{1,4}\)?[\s\-]?\d{1,4}[\s\-]?\d{1,9}$/.test(v.trim()),
...
const messageSchema = z
  ...
  .refine(v => v.trim().length >= 10, 'Message must contain at least 10 meaningful characters')
  .transform(v => v.trim());
...
export const fieldSchemas = { name: nameSchema, email: emailSchema, phone: phoneSchema, subject: subjectSchema, message: messageSchema } as const;
...
export function validateField(field: keyof ContactFormData, value: unknown) {
  try {
    ...
    const issue = err instanceof z.ZodError ? err.issues[0]?.message : 'Validation error occurred';
...
// Utility helpers
export const isFormReadyForSubmission = (data: unknown) => validateContactForm(data).success;
...
export const fieldRequirements = {
  name: { required: true, minLength: 2, maxLength: 100, pattern: 'Letters, spaces, hyphens, and apostrophes only' },
  email: { required: true, maxLength: 254, pattern: 'Valid email address format' },
  phone: { required: false, pattern: 'International format (e.g., +1-234-567-8900)' },
